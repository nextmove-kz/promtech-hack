...
export function ActionPlanModal({
  ...
  const generateActionPlan = useCallback(async () => {
    ...
    try {
      ...
      // Save plan to PocketBase after successful generation
      if (data.result && diagnostic.object) {
        await savePlanToPocketBase(data.result, diagnostic.object);
      }
    } catch (err) {
      setGenerationError(
  ...
  }, [diagnosticId, diagnostic, savePlanToPocketBase]);
