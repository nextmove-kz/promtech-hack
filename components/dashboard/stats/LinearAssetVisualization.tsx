...
export function LinearAssetVisualization() {
  ...
  return (
    <Card className="col-span-full">
      <CardHeader>
        <CardTitle className="text-xl">ЭКГ Магистрали (Linear Asset Health)</CardTitle>
        <CardDescription>
          Визуализация уровня риска по протяженности трубопровода •
          Средний риск: <span className="font-semibold">{avgRisk.toFixed(1)}</span> •
          Пик риска: <span className="font-semibold text-red-500">{hotspot?.km.toFixed(1)} км</span>
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <XAxis
                dataKey="km"
                label={{ value: 'Километраж (км)', position: 'insideBottom', offset: -5 }}
                className="text-xs"
              />
              <YAxis
                label={{ value: 'Уровень риска', angle: -90, position: 'insideLeft' }}
                domain={[0, 100]}
                className="text-xs"
              />
              <Tooltip
                ...
                formatter={(value: number, name: string, props: any) => {
                  ...
                  return [
                    <div key="tooltip" className="flex flex-col gap-1">
                      <span className="font-semibold">Риск: {value.toFixed(1)}</span>
                      {objectName && <span className="text-xs text-muted-foreground">{objectName}</span>}
                    </div>,
                    ''
                  ]
                }}
              ...
              <ReferenceLine
                ...
                strokeDasharray="3 3"
                label={{ value: 'Критический порог', position: 'right', fill: 'hsl(var(--destructive))' }}
              />
              <ReferenceLine
                y={40}
                stroke="hsl(var(--warning))"
                strokeDasharray="3 3"
                label={{ value: 'Порог предупреждения', position: 'right', fill: 'hsl(var(--warning))' }}
              />
              <Area
                type="monotone"
                dataKey="risk"
        ...
        {processedData.length > 0 && (
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            <div className="rounded-lg border border-border bg-muted/50 p-3">
              <div className="text-xs text-muted-foreground">Протяженность мониторинга</div>
              <div className="text-lg font-semibold">
                {Math.max(...processedData.map(d => d.km), 0).toFixed(1)} км
              </div>
            </div>
            <div className="rounded-lg border border-border bg-muted/50 p-3">
              <div className="text-xs text-muted-foreground">Критических участков</div>
