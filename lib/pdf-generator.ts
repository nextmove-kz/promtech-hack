...
export async function generateActionPlanPdf(
  ...
  if (object_data.last_diagnostic) {
    ...
    if (diag.ml_label) {
      const label =
        diag.ml_label === 'normal'
          ? 'Норма'
          : diag.ml_label === 'medium'
            ? 'Средний риск'
            : 'Высокий риск';
    ...
    if (
      ...
      drawField('Параметр 1:', params.param1 as string);
    if (
      params.param2 !== undefined &&
      params.param2 !== null &&
      params.param2 !== 0
    )
      drawField('Параметр 2:', params.param2 as string);
    if (
      params.param3 !== undefined &&
      params.param3 !== null &&
      params.param3 !== 0
    )
      drawField('Параметр 3:', params.param3 as string);
  }
