...
export default function PlanPage() {
  ...
  const [_localActionStates, setLocalActionStates] = useState<
  ...
  const handleMarkAsDoneClick = () => {
    ...
    const allActionsCompleted = actions.every((action) => action.status);
  ...
  const actions = plan.expand?.actions || [];
  const completedCount = actions.filter((a) => a.status).length;
              ...
              {actions.map((action) => {
                ...
                return (
                  <button
                    ...
                    disabled={isDisabled}
                    className={cn(
                      'w-full flex items-start gap-3 rounded-lg border border-border bg-card p-4 text-left transition-all',
