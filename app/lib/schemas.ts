...
// Enum definitions
export const ObjectTypeEnum = z.enum(["crane", "compressor", "pipeline_section"]);
...
export function detectDataType(row: Record<string, unknown>): DataType | null {
  ...
  const hasDiagnosticHeaders = DIAGNOSTIC_REQUIRED_HEADERS.every((h) => keys.includes(h));
  if (hasDiagnosticHeaders) {
    return "diagnostics";
  }

  // Check for objects
  const hasObjectHeaders = OBJECT_REQUIRED_HEADERS.every((h) => keys.includes(h));
  if (hasObjectHeaders) {
    return "objects";
  }
...
export function validateRow(
  ...
  dataType: DataType
): { success: true; data: ObjectRow | DiagnosticRow } | { success: false; errors: string[] } {
