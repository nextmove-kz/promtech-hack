...
function generateDiagnostics(
  ...
  let currentTotal = Array.from(objectDiagCounts.values()).reduce((a, b) => a + b, 0);
  ...
  for (const obj of objects) {
    ...
    for (let i = 0; i < numDiag; i++) {
      ...
      } else if (objectHealth === "medium" && (needsRequiredSeverity || random() < 0.5)) {
...
function getObjectHealthStats(
  ...
  for (const diag of diagnostics) {
    ...
    } else if (diagLabel === "high" || (diagLabel === "medium" && current === "normal")) {
      objectWorstHealth.set(diag.object_id, diagLabel);
    }
  }
  
  let good = 0, medium = 0, bad = 0, noDiagnostics = 0;
...
const HEALTH_DISTRIBUTION = { good: 0.30, medium: 0.50, bad: 0.20 };
...
for (const pipeline of PIPELINES) {
  const route = PIPELINE_ROUTES[pipeline.id];
  console.log(`   ${pipeline.id}: ${pipeline.name} (${route.length} waypoints, weight: ${(pipeline.weight * 100).toFixed(0)}%)`);
}
...
const diagnostics = generateDiagnostics(objects, TOTAL_DIAGNOSTICS, HEALTH_DISTRIBUTION);
...
console.log(`   📈 Diagnostic risk levels: High=${highRisk}, Medium=${mediumRisk}, Normal=${normalRisk}`);

// Object health statistics
const healthStats = getObjectHealthStats(objects, diagnostics);
console.log(`   🏥 Object health: Good=${healthStats.good} (${((healthStats.good/objects.length)*100).toFixed(0)}%), Medium=${healthStats.medium} (${((healthStats.medium/objects.length)*100).toFixed(0)}%), Bad=${healthStats.bad} (${((healthStats.bad/objects.length)*100).toFixed(0)}%)`);
...
console.log(`   📍 Bounds: lat ${SMALL_DATASET_BOUNDS.latMin.toFixed(4)}-${SMALL_DATASET_BOUNDS.latMax.toFixed(4)}, lon ${SMALL_DATASET_BOUNDS.lonMin.toFixed(4)}-${SMALL_DATASET_BOUNDS.lonMax.toFixed(4)}`);

const smallObjects = generateObjectsInBounds(SMALL_OBJECTS, SMALL_DATASET_BOUNDS, "MT-03", 99);
...
writeFileSync(smallDiagnosticsPath, diagnosticsToCsv(renumberedSmallDiagnostics), "utf-8");
console.log(`   ✅ ${smallDiagnosticsPath} (${renumberedSmallDiagnostics.length} diagnostics)`);

// Small dataset health stats
const smallHealthStats = getObjectHealthStats(smallObjects, renumberedSmallDiagnostics);
console.log(`   🏥 Small dataset health: Good=${smallHealthStats.good}, Medium=${smallHealthStats.medium}, Bad=${smallHealthStats.bad}`);
